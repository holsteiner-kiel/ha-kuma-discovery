...
class ImageMetadataTests(unittest.TestCase):
    def setUp(self):
        self.config = {'version': '2.0.1', 'arch': ['amd64', 'aarch64'],
    ...
    def test_release_strips_v_by_using_app_version(self):
        self.assertEqual(self.check('release', release_tag='v2.0.1'), ('2.0.1', True))
        with self.assertRaises(ValueError):
            self.check('release', release_tag='v2.0.2')

    def test_pr_and_fork_cannot_publish(self):
        self.assertFalse(self.check('pull_request', request_publish='true')[1])
        self.assertFalse(self.check('release', repo='example/fork', release_tag='v2.0.1')[1])
    ...
    def test_branch_pushes_cannot_publish(self):
        ...
        self.assertFalse(self.check('push', ref='refs/heads/release/2.0.1')[1])
