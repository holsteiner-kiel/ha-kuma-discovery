...
class ImageMetadataTests(unittest.TestCase):
    def setUp(self):
        self.config = {'version': '1.5.2', 'arch': ['amd64', 'aarch64'],
    ...
    def test_release_strips_v_by_using_app_version(self):
        self.assertEqual(self.check('release', release_tag='v1.5.2'), ('1.5.2', True))
        with self.assertRaises(ValueError):
            self.check('release', release_tag='v1.6.0')

    def test_pr_and_fork_cannot_publish(self):
        self.assertFalse(self.check('pull_request', request_publish='true')[1])
        self.assertFalse(self.check('release', repo='example/fork', release_tag='v1.5.2')[1])
